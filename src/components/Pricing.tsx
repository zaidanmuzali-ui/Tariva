  ...
  const handleUpgradeToPro = async () => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
      const response = await fetch('/api/payment/checkout', {
        ...
        body: JSON.stringify({
          planName: `Tariva Pro Plan (${billingCycle === 'monthly' ? 'Bulanan' : 'Tahunan'})`
        })
      });
      ...
      if (data.token) {
        // Store orderId for manual sync fallback if webhook lags
        if (user) {
          localStorage.setItem(`last_order_${user.id}`, data.orderId);
        }

        // Use Midtrans Snap SDK
        if (!(window as any).snap) {
          toast.error('Gagal memuat sistem pembayaran. Silakan muat ulang halaman.');
          setLoadingPro(false);
          return;
        }
        ...
        (window as any).snap.pay(data.token, {
          onSuccess: async (result: any) => {
            ...
            toast.success('Pembayaran Terkonfirmasi! Sedang menyinkronkan akun...');
            ...
            try {
              ...
              if (session?.user?.user_metadata?.plan === 'pro') {
                toast.success('Akun Anda telah berhasil di-upgrade ke PRO!');
                setTimeout(() => window.location.reload(), 1500);
              } else {
                toast.info('Pembaruan status sedang diproses. Halaman akan dimuat ulang otomatis.');
          ...
          onPending: (result: any) => {
            ...
            toast.info('Menunggu penyelesaian pembayaran. Silakan cek aplikasi mobile banking atau e-wallet Anda.');
            setLoadingPro(false);
          },
          onError: (result: any) => {
            console.error('Payment Error:', result);
            toast.error('Gagal memproses pembayaran. Silakan coba lagi atau gunakan metode lain.');
            setLoadingPro(false);
          },
          onClose: () => {
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 md:py-20 space-y-16 md:space-y-24">
      <motion.section {...sectionAnimate} className="text-center space-y-6 md:space-y-8">
        ...
        <p className="text-base md:text-lg text-slate-500 max-w-2xl mx-auto px-4">
          Mulai gratis untuk pencarian dasar, upgrade ke Pro untuk fitur analisis mendalam dan dokumen lengkap.
        </p>
            ...
            <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] border-2 border-primary-brand overflow-hidden shadow-xl shadow-primary-brand/5 flex flex-col h-full ring-4 ring-blue-600/5">
              ...
              <div className="p-6 md:p-10 pt-0 mt-auto text-center">
                ...
                >
                  ...
                  {isPro ? 'Paket Pro Aktif' : `Mulai Pro — Rp ${currentPrice.toLocaleString('id-ID')}`}
                </Button>
                {!isPro && (
